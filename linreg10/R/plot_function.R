...
#' @return A \code{data frame} c("res_fit", "qq", "hist, all"), res_fit -> Residuals vs fitted-values, qq -> qq-plot of residuals, hist -> Histogram of residuals, all -> all of the plots together
#' \describe{
#'      }
...
plots <- function(Response,Predictors,pl_type = "hist"){
  n <-  length(Response)
  ...
  if(pl_type == "res_fit") {
    ...
    print(ggplot(pl_dat, aes(x = Residuals)) + geom_density() +
    ...
    d <- ggplot(pl_dat, aes(x = Residuals)) + geom_density() +
      labs(title = "Density Plot of Residuals")

    print(plot_grid(a,b,c,d))
